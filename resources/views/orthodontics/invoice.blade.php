...
@php
    ...
    $clinic = $case->clinic;
    $currency = $case->currency ?: 'USD';
    $paymentPercentage = $case->total_cost > 0 ? min(100, ($case->paid_amount / $case->total_cost) * 100) : 0;
    $logoSrc = $clinic?->id ? \App\Helpers\ClinicHelper::getClinicLogoPdfSrc($clinic->id) : null;
@endphp
<html lang="{{ app()->getLocale() }}">
<head>
    <meta charset="UTF-8">
    <title>{{ __('Orthodontic Invoice') }} - {{ $case->case_number }}</title>
    <style>
...
<body>
    <div class="toolbar">
        ...
        <a href="{{ route('orthodontics.receipt', [$case, 'width' => 80, 'auto' => 0]) }}">{{ __('Thermal 80mm') }}</a>
        <a href="{{ route('orthodontics.receipt', [$case, 'width' => 58, 'auto' => 0]) }}">{{ __('Thermal 58mm') }}</a>
    ...
    <div class="invoice">
        <div class="header">
            ...
            <div class="header-right">
                <h1>{{ __('Orthodontic Invoice') }}</h1>
                <div><strong>{{ __('Case') }}:</strong> {{ $case->case_number }}</div>
        ...
        <div class="grid">
            <div class="col">
                <h2>{{ __('Patient / Case Information') }}</h2>
                <div class="row"><span class="label">{{ __('Patient') }}</span><span class="value">{{ $case->patient->full_name ?? '-' }}</span></div>
                <div class="row"><span class="label">{{ __('Patient ID') }}</span><span class="value">{{ $case->patient->patient_id ?? '-' }}</span></div>
                <div class="row"><span class="label">{{ __('Doctor') }}</span><span class="value">{{ $case->doctor->full_name ?? '-' }}</span></div>
                <div class="row"><span class="label">{{ __('Treatment') }}</span><span class="value">{{ $case->treatment_type_display }}</span></div>
            </div>
            <div class="col">
                <h2>{{ __('Treatment Details') }}</h2>
                <div class="row"><span class="label">{{ __('Start Date') }}</span><span class="value">{{ optional($case->start_date)->format('Y-m-d') }}</span></div>
                <div class="row"><span class="label">{{ __('Duration') }}</span><span class="value">{{ $case->estimated_duration_months }} {{ __('months') }}</span></div>
                <div class="row"><span class="label">{{ __('Current Phase') }}</span><span class="value">{{ $case->current_phase_display ?: '-' }}</span></div>
                <div class="row"><span class="label">{{ __('Status') }}</span><span class="value">{{ $case->status_display }}</span></div>
            </div>
        </div>

        <h2>{{ __('Financial Summary') }}</h2>
        <div class="summary">
            <div class="row"><span class="label">{{ __('Total Cost') }}</span><span class="value amount-total">{{ number_format($case->total_cost, 2) }} {{ $currency }}</span></div>
            <div class="row"><span class="label">{{ __('Paid Amount') }}</span><span class="value amount-paid">{{ number_format($case->paid_amount, 2) }} {{ $currency }}</span></div>
            <div class="row"><span class="label">{{ __('Balance') }}</span><span class="value amount-balance">{{ number_format($case->balance, 2) }} {{ $currency }}</span></div>
            <div class="row"><span class="label">{{ __('Payment Plan') }}</span><span class="value">{{ \App\Models\OrthodonticCase::PAYMENT_PLANS[$case->payment_plan] ?? $case->payment_plan }}</span></div>
        ...
        <table>
            <thead><tr><th>{{ __('Date') }}</th><th>{{ __('Type') }}</th><th>{{ __('Method') }}</th><th>{{ __('Receipt #') }}</th><th class="right">{{ __('Amount') }}</th></tr></thead>
            <tbody>
            @forelse($case->payments->sortBy('payment_date') as $payment)
                <tr>
                    <td>{{ optional($payment->payment_date)->format('Y-m-d') }}</td><td>{{ $payment->payment_type_display }}</td><td>{{ $payment->payment_method_display }}</td><td>{{ $payment->receipt_number ?: '-' }}</td><td class="right">{{ number_format($payment->amount, 2) }} {{ $payment->currency }}</td>
                </tr>
            @empty
                <tr><td colspan="5" class="muted">{{ __('No payments recorded yet.') }}</td></tr>
            @endforelse
            </tbody>
        </table>
        <div class="footer">{{ __('Thank you') }} — {{ config('app.name') }}</div>
    </div>
