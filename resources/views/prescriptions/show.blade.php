                ...
                <div>
                    ...
                    <a href="{{ route('messages.index') }}?prefill_transfer={{ urlencode(base64_encode(json_encode([
                       ...
                       onclick="try{var v=JSON.stringify({
                        ...
                        <button type="button" class="btn btn-success me-2" onclick="sharePrescriptionWhatsApp()">
...
function sharePrescriptionWhatsApp() {
  ...
  @if(isset($prescription->medicines) && $prescription->medicines->count() > 0)
    ...
    @foreach($prescription->medicines as $m)
      meds += "• {{ addslashes($m->name) }}" + ("{{ $m->pivot->strength ?? $m->strength ? ' — ' . addslashes($m->pivot->strength ?? $m->strength) : '' }}") +
              ("{{ $m->pivot->dosage ? ' · ' . addslashes($m->pivot->dosage) : '' }}") +
              ("{{ $m->pivot->frequency ? ' · ' . addslashes(ucfirst(str_replace('_',' ',$m->pivot->frequency))) : '' }}") +
              ("{{ $m->pivot->duration ? ' · ' . addslashes($m->pivot->duration) : '' }}") +
              ("{{ $m->pivot->instructions ? ' · ' . addslashes($m->pivot->instructions) : '' }}") + "\n";
    @endforeach
  @endif
