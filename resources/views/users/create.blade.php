                                ...
                                <div class="row g-3">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <select class="form-select @error('role') is-invalid @enderror" id="role" name="role" required onchange="updateRoleDescription()">
                                            ...
                                            @if(in_array('dental_dept', $availableRoles ?? []))
                                                <option value="dental_dept" {{ old('role', request('assign_to') ? 'assistant' : '') == 'dental_dept' ? 'selected' : '' }}>{{ __('Dentist') }}</option>
                                            @endif
                                            @if(in_array('dental_technician', $availableRoles ?? []))
                                                <option value="dental_technician" {{ old('role', request('assign_to') ? 'assistant' : '') == 'dental_technician' ? 'selected' : '' }}>{{ __('Dental Technician') }}</option>
                                            @endif
                                            @if(in_array('cad_cam_designer', $availableRoles ?? []))
                                                <option value="cad_cam_designer" {{ old('role', request('assign_to') ? 'assistant' : '') == 'cad_cam_designer' ? 'selected' : '' }}>{{ __('CAD/CAM Designer') }}</option>
                                            @endif
                                    ...
                                    <div class="col-12 mt-4" id="systemPermissions">
                                        ...
                                        <div class="alert alert-info mb-4">
                                            ...
                                            {{ __('Select the permissions below as needed. Role does not auto-assign permissions; admins must choose access explicitly for each user.') }}
                                        </div>
