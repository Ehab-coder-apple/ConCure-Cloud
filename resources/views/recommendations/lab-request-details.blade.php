...
function sendViaWhatsApp(labRequestId) {
    // Build WhatsApp message exactly like Nutrition Plan style (client-side wa.me link)
    const patientName = "{{ $labRequest->patient->full_name }}";
    const patientId = "{{ $labRequest->patient->patient_id }}";
    const requestNumber = "{{ $labRequest->request_number }}";
    const doctorName = "{{ $labRequest->doctor ? ($labRequest->doctor->first_name . ' ' . $labRequest->doctor->last_name) : 'Unknown' }}";
    const dueDate = "{{ $labRequest->due_date ? $labRequest->due_date->format('M d, Y') : '' }}";

    let testsSummary = "";
    @if($labRequest->tests && $labRequest->tests->count() > 0)
        testsSummary += "\n🧪 Tests Required:\n";
        @foreach($labRequest->tests as $t)
            testsSummary += "• {{ addslashes($t->test_name) }}" + ("{{ $t->instructions ? ' — ' . addslashes($t->instructions) : '' }}") + "\n";
        @endforeach
    @endif

    const message = `🧪 Lab Request\n\n👤 Patient: ${patientName} (${patientId})\n📄 Request #: ${requestNumber}\n👨‍⚕️ Doctor: ${doctorName}${dueDate ? `\n📅 Due: ${dueDate}` : ''}\n${testsSummary}\n📱 Generated by ConCure Clinic Management System`;

    const encoded = encodeURIComponent(message);

    // Prefer lab WhatsApp number; fall back to open composer without number
    const labWhatsApp = '{{ $labRequest->lab_whatsapp ? preg_replace("/[^0-9]/", "", $labRequest->lab_whatsapp) : '' }}';
    const url = labWhatsApp ? `https://wa.me/${labWhatsApp}?text=${encoded}` : `https://wa.me/?text=${encoded}`;
