    ...
    <div class="row">
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center mb-4">
                ...
                <div>
                    <a href="{{ route('recommendations.radiology.index') }}" class="btn btn-secondary me-2">
...
function shareRadiologyWhatsApp() {
  const patientName = "{{ $radiologyRequest->patient?->full_name ?? '' }}";
  const patientId = "{{ $radiologyRequest->patient?->patient_id ?? '' }}";
  const requestNumber = "{{ $radiologyRequest->request_number }}";
  const doctorName = "{{ $radiologyRequest->doctor?->full_name ?? '' }}";
  const dueDate = "{{ $radiologyRequest->due_date ? $radiologyRequest->due_date->format('M d, Y') : '' }}";

  let testsSummary = "";
  @if($radiologyRequest->tests && $radiologyRequest->tests->count() > 0)
    testsSummary += "\n🧪 {{ __('Tests Required') }}:\n";
    @foreach($radiologyRequest->tests as $t)
      testsSummary += "• {{ addslashes($t->test_name_display) }}" + ("{{ $t->instructions ? ' — ' . addslashes($t->instructions) : '' }}") + "\n";
    @endforeach
  @endif

  const message = `🩻 {{ __('Radiology Request') }}\n\n` +
                  `👤 {{ __('Patient') }}: ${patientName} ${patientId ? '('+patientId+')' : ''}\n` +
                  `📄 {{ __('Request #') }}: ${requestNumber}\n` +
                  `${doctorName ? '👨‍⚕️ {{ __('Doctor') }}: ' + doctorName + '\n' : ''}` +
                  `${dueDate ? '📅 {{ __('Due') }}: ' + dueDate + '\n' : ''}` +
                  `\n` + testsSummary +
                  `\n📱 {{ __('Generated by ConCure Clinic Management System') }}`;

  const encoded = encodeURIComponent(message);
  const centerWhatsApp = '{{ $radiologyRequest->radiology_center_whatsapp ? preg_replace('/[^0-9]/', '', $radiologyRequest->radiology_center_whatsapp) : '' }}';
  const url = centerWhatsApp ? `https://wa.me/${centerWhatsApp}?text=${encoded}` : `https://wa.me/?text=${encoded}`;
