...
@section('page-title', __('Edit Radiology Request'))
...
<div class="container">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h1 class="h3 mb-0">
                        ...
                        {{ __('Edit Radiology Request') }} - {{ $radiologyRequest->request_number }}
                    </h1>
                    <p class="text-muted mb-0">{{ __('Update radiology and imaging tests for your patient') }}</p>
                </div>
                <div>
                    <a href="{{ route('recommendations.radiology.show', $radiologyRequest) }}" class="btn btn-outline-secondary">
    ...
    <form action="{{ route('recommendations.radiology.update', $radiologyRequest) }}" method="POST" id="radiology-form">
        @csrf
        @method('PUT')
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select @error('patient_id') is-invalid @enderror" id="patient_id" name="patient_id" required>
                                    ...
                                    <option value="{{ $patient->id }}" 
                                            {{ (old('patient_id', $radiologyRequest->patient_id) == $patient->id) ? 'selected' : '' }}>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select @error('priority') is-invalid @enderror" id="priority" name="priority" required>
                                    ...
                                    <option value="{{ $key }}" {{ old('priority', $radiologyRequest->priority) === $key ? 'selected' : '' }}>{{ __($label) }}</option>
                                    @endforeach
                                </select>
                                @error('priority')
                                <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('suspected_diagnosis') is-invalid @enderror" 
                                   id="suspected_diagnosis" name="suspected_diagnosis" value="{{ old('suspected_diagnosis', $radiologyRequest->suspected_diagnosis) }}"
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <textarea class="form-control @error('clinical_notes') is-invalid @enderror" 
                                          ...
                                          placeholder="{{ __('Clinical findings and relevant information...') }}">{{ old('clinical_notes', $radiologyRequest->clinical_notes) }}</textarea>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <textarea class="form-control @error('clinical_history') is-invalid @enderror" 
                                          ...
                                          placeholder="{{ __('Relevant medical history...') }}">{{ old('clinical_history', $radiologyRequest->clinical_history) }}</textarea>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" class="form-control @error('due_date') is-invalid @enderror" 
                                   id="due_date" name="due_date" value="{{ old('due_date', $radiologyRequest->due_date?->format('Y-m-d')) }}" min="{{ date('Y-m-d', strtotime('+1 day')) }}">
                            @error('due_date')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div id="tests-container">
                            ...
                            <div class="test-item border rounded p-3 mb-3" data-index="{{ $index }}">
                                <div class="d-flex justify-content-between align-items-start mb-2">
                                    <h6 class="mb-0">{{ __('Test') }} #{{ $index + 1 }}</h6>
                                    <button type="button" class="btn btn-sm btn-outline-danger remove-test" onclick="removeTest({{ $index }})" {{ $radiologyRequest->tests->count() <= 1 ? 'style=display:none;' : '' }}>
                                ...
                                <div class="row">
                                    <div class="col-md-8 mb-3">
                                        ...
                                        <div class="input-group">
                                            <input type="text" class="form-control test-name" name="tests[{{ $index }}][test_name]"
                                                   value="{{ old('tests.'.$index.'.test_name', $test->test_name) }}"
                                                   placeholder="{{ __('Enter test name or select from database') }}" required>
                                            <button type="button" class="btn btn-outline-secondary" onclick="showTestSelector({{ $index }})">
                                                <i class="fas fa-search"></i>
                                            </button>
                                        </div>
                                        <input type="hidden" name="tests[{{ $index }}][radiology_test_id]" class="radiology-test-id"
                                               value="{{ old('tests.'.$index.'.radiology_test_id', $test->radiology_test_id) }}">
                                    ...
                                    <div class="col-md-4 mb-3">
                                        ...
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" name="tests[{{ $index }}][with_contrast]" value="1" 
                                                   {{ old('tests.'.$index.'.with_contrast', $test->with_contrast) ? 'checked' : '' }}>
                                            <label class="form-check-label">{{ __('With Contrast') }}</label>
                                        </div>
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" name="tests[{{ $index }}][urgent]" value="1" 
                                                   {{ old('tests.'.$index.'.urgent', $test->urgent) ? 'checked' : '' }}>
                                ...
                                <div class="row">
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <input type="text" class="form-control" name="tests[{{ $index }}][clinical_indication]" 
                                               value="{{ old('tests.'.$index.'.clinical_indication', $test->clinical_indication) }}"
                                    ...
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <input type="text" class="form-control" name="tests[{{ $index }}][instructions]" 
                                               value="{{ old('tests.'.$index.'.instructions', $test->instructions) }}"
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control" name="tests[{{ $index }}][special_requirements]" rows="2"
                                              placeholder="{{ __('Any special requirements or preparations') }}">{{ old('tests.'.$index.'.special_requirements', $test->special_requirements) }}</textarea>
                                </div>
                            </div>
                            @endforeach
                        </div>
                    ...
                    <div class="card-body">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('radiology_center_name') is-invalid @enderror" 
                                   id="radiology_center_name" name="radiology_center_name" value="{{ old('radiology_center_name', $radiologyRequest->radiology_center_name) }}"
                        ...
                        <div class="mb-3">
                            ...
                            <input type="tel" class="form-control @error('radiology_center_phone') is-invalid @enderror" 
                                   id="radiology_center_phone" name="radiology_center_phone" value="{{ old('radiology_center_phone', $radiologyRequest->radiology_center_phone) }}"
                        ...
                        <div class="mb-3">
                            ...
                            <input type="tel" class="form-control @error('radiology_center_whatsapp') is-invalid @enderror" 
                                   id="radiology_center_whatsapp" name="radiology_center_whatsapp" value="{{ old('radiology_center_whatsapp', $radiologyRequest->radiology_center_whatsapp) }}"
                        ...
                        <div class="mb-3">
                            ...
                            <input type="email" class="form-control @error('radiology_center_email') is-invalid @enderror" 
                                   id="radiology_center_email" name="radiology_center_email" value="{{ old('radiology_center_email', $radiologyRequest->radiology_center_email) }}"
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('radiology_center_address') is-invalid @enderror" 
                                      ...
                                      placeholder="{{ __('Center address') }}">{{ old('radiology_center_address', $radiologyRequest->radiology_center_address) }}</textarea>
                    ...
                    <div class="card-body">
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('notes') is-invalid @enderror" 
                                      ...
                                      placeholder="{{ __('Any additional notes or instructions...') }}">{{ old('notes', $radiologyRequest->notes) }}</textarea>
                            @error('notes')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
...
@push('scripts')
<script>
let testIndex = {{ $radiologyRequest->tests->count() }};
