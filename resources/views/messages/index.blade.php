          ...
          <div class="card">
            <div class="card-body">
              <div class="d-flex justify-content-between">
                <div>
                  <div class="fw-semibold">${t.metadata && t.metadata.quick_visit === true ? 'New Visit Sent for Review' : 'Transfer: ' + t.transfer_type.replace('_',' ')}</div>
              ...
              <div class="mt-2">
                ...
                ${(() => { let label=null, inner=null; switch(t.transfer_type){ case 'lab_request': if(t.source_id){label='Open Lab Request'; inner=`<a class=\"btn btn-sm btn-outline-dark\" href=\"/recommendations/lab-requests/${t.source_id}\" target=\"_blank\">${label}</a>`;} break; case 'radiology_request': if(t.source_id){label='Open Radiology Request'; inner=`<a class=\"btn btn-sm btn-outline-dark\" href=\"/recommendations/radiology/${t.source_id}\" target=\"_blank\">${label}</a>`;} break; case 'prescription': if(t.source_id){ if (t.metadata && t.metadata.quick_visit === true) { label='Preview'; inner=`<a class=\"btn btn-sm btn-primary\" href=\"/simple-prescriptions/quick-visit/${t.source_id}/review\">${label}</a>`; } else { label='Open Prescription'; const isSimple = !!(t && t.metadata && (t.metadata.simple === true || t.metadata.type === 'simple')); inner=`<button class=\"btn btn-sm btn-outline-dark\" onclick=\"openPrescriptionSmart(${t.source_id}, ${isSimple ? 'true' : 'false'})\">${label}</button>`; } } break; case 'nutrition_plan': if(t.source_id){label='Open Nutrition Plan'; inner=`<a class=\"btn btn-sm btn-outline-dark\" href=\"/nutrition/${t.source_id}\" target=\"_blank\">${label}</a>`;} break;} return inner ? `<div class=\"mt-2\">${inner}</div>` : '' })()}
