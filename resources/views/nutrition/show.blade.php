    ...
    try {
        ...
        @foreach(['breakfast', 'lunch', 'dinner', 'snack_1', 'snack_2', 'snack_3', 'snack'] as $mealType)
            @if(isset($mealsByType[$mealType]) && $mealsByType[$mealType]->count() > 0)
                message += '{{ $mealTypeLabels[$mealType] ?? ucfirst($mealType) }}:\n';
                @foreach($mealsByType[$mealType] as $meal)
                    @if($meal->foods->count() > 0)
                        @foreach($meal->foods as $food)
                            message += '• {{ addslashes($food->food_name_display) }} - {{ $food->quantity }}{{ $food->unit }}\n';
                        @endforeach
                    @endif
                @endforeach
                message += '\n';
        ...
        @if($dietPlan->target_calories)
            ...
            message += '🔥 Calories: {{ $dietPlan->target_calories }}\n';
            @if($dietPlan->target_protein)
                message += '🥩 Protein: {{ $dietPlan->target_protein }}g\n';
            @endif
            @if($dietPlan->target_carbs)
                message += '🍞 Carbs: {{ $dietPlan->target_carbs }}g\n';
            @endif
            @if($dietPlan->target_fat)
                message += '🥑 Fat: {{ $dietPlan->target_fat }}g\n';
            @endif
            message += '\n';
        @endif

        message += '💡 *Instructions:*\n';
        message += 'Follow this personalized nutrition plan as prescribed by your doctor. ';
        message += 'For any questions or concerns, please contact your healthcare provider.\n\n';
