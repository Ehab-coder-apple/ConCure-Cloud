                                            ...
                                            <div class="card border shadow-sm h-100">
                                                ...
                                                <div class="card-body">
                                                    ...
                                                    @elseif($module['key'] === 'orthodontics')
                                                        <div class="text-muted text-center py-4"><i class="fas fa-teeth fa-2x mb-2 d-block text-primary"></i><p class="mb-0 small">{{ __('Orthodontic cases are managed separately from patient profile') }}</p><a href="{{ route('orthodontics.index', ['patient_id' => $patient->id]) }}" class="btn btn-sm btn-outline-primary mt-2"><i class="fas fa-arrow-right me-1"></i>{{ __('Go to Orthodontic Cases') }}</a></div>
                                                    @elseif($module['key'] === 'pediatric')
                                                        @php($latestGrowth = $patient->growthMeasurements()->orderByDesc('measurement_date')->first())
                                                        <div class="row g-3"><div class="col-md-6"><label for="pediatric_birth_weight" class="form-label">{{ __('Birth Weight') }} <span class="text-muted small">({{ __('grams') }})</span></label><input type="number" id="pediatric_birth_weight" name="pediatric_birth_weight" min="200" max="7000" step="1" class="form-control module-field @error('pediatric_birth_weight') is-invalid @enderror" value="{{ old('pediatric_birth_weight', $patient->pediatricProfile?->birth_weight) }}" {{ $isSelected ? '' : 'disabled' }}>@error('pediatric_birth_weight')<div class="invalid-feedback">{{ $message }}</div>@enderror</div><div class="col-md-6"><label for="pediatric_gestational_age_weeks" class="form-label">{{ __('Gestational Age') }} <span class="text-muted small">({{ __('weeks') }})</span></label><input type="number" id="pediatric_gestational_age_weeks" name="pediatric_gestational_age_weeks" min="20" max="45" step="1" class="form-control module-field @error('pediatric_gestational_age_weeks') is-invalid @enderror" value="{{ old('pediatric_gestational_age_weeks', $patient->pediatricProfile?->gestational_age) }}" {{ $isSelected ? '' : 'disabled' }}>@error('pediatric_gestational_age_weeks')<div class="invalid-feedback">{{ $message }}</div>@enderror</div><div class="col-12"><div class="border-top pt-3 mt-1"><div class="d-flex align-items-center justify-content-between gap-2 mb-2"><div class="d-flex align-items-center gap-2"><i class="fas fa-chart-line text-success"></i><span class="fw-semibold small text-uppercase text-muted">{{ __('Current Measurement (for growth chart)') }}</span></div>@if($latestGrowth)<span class="badge bg-light text-muted small"><i class="fas fa-history me-1"></i>{{ __('Latest') }}: {{ $latestGrowth->measurement_date->format('M d, Y') }}</span>@endif</div><div class="row g-3"><div class="col-md-4"><label for="pediatric_current_weight_kg" class="form-label">{{ __('Weight (kg)') }}</label><input type="number" id="pediatric_current_weight_kg" name="pediatric_current_weight_kg" min="0" max="300" step="0.001" class="form-control module-field @error('pediatric_current_weight_kg') is-invalid @enderror" value="{{ old('pediatric_current_weight_kg') }}" placeholder="{{ $latestGrowth?->weight_kg ? number_format((float) $latestGrowth->weight_kg, 3) : 'e.g. 3.250' }}" {{ $isSelected ? '' : 'disabled' }}>@error('pediatric_current_weight_kg')<div class="invalid-feedback">{{ $message }}</div>@enderror</div><div class="col-md-4"><label for="pediatric_current_height_cm" class="form-label">{{ __('Length/Height (cm)') }}</label><input type="number" id="pediatric_current_height_cm" name="pediatric_current_height_cm" min="0" max="250" step="0.01" class="form-control module-field @error('pediatric_current_height_cm') is-invalid @enderror" value="{{ old('pediatric_current_height_cm') }}" placeholder="{{ $latestGrowth?->length_height_cm ? number_format((float) $latestGrowth->length_height_cm, 2) : 'e.g. 50.00' }}" {{ $isSelected ? '' : 'disabled' }}>@error('pediatric_current_height_cm')<div class="invalid-feedback">{{ $message }}</div>@enderror</div><div class="col-md-4"><label for="pediatric_head_circumference_cm" class="form-label">{{ __('Head Circ. (cm)') }}</label><input type="number" id="pediatric_head_circumference_cm" name="pediatric_head_circumference_cm" min="0" max="80" step="0.01" class="form-control module-field @error('pediatric_head_circumference_cm') is-invalid @enderror" value="{{ old('pediatric_head_circumference_cm') }}" placeholder="{{ $latestGrowth?->head_circumference_cm ? number_format((float) $latestGrowth->head_circumference_cm, 2) : 'e.g. 35.00' }}" {{ $isSelected ? '' : 'disabled' }}>@error('pediatric_head_circumference_cm')<div class="invalid-feedback">{{ $message }}</div>@enderror</div></div><div class="form-text small mt-1"><i class="fas fa-info-circle me-1"></i>{{ __('Optional. If filled, a new growth measurement is recorded with today\'s date.') }} <a href="{{ route('pediatric.growth-chart', $patient) }}" class="text-decoration-none">{{ __('Open growth chart') }} <i class="fas fa-arrow-right ms-1"></i></a></div></div></div><div class="col-12 d-none" id="pediatric-status-indicator"><div id="pediatric-status-message" class="alert py-2 mb-0"></div></div></div>
