...
@php
    $selectedModules = collect(old('selected_modules', $selectedModuleKeys ?? []))
        ->merge([
    ...
    $medicalFlagsState = $patient->medical_flags ?? [];
@endphp
...
<div class="container-fluid py-3 py-lg-4">
    <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-4">
        <div>
            <h1 class="h3 mb-1"><i class="fas fa-user-edit text-primary me-2"></i>{{ __('Edit Patient') }}</h1>
            <p class="text-muted mb-0">{{ __('Update the shared patient profile, adjust medical overview flags, and manage optional specialty modules from one place.') }}</p>
        </div>
        <div class="d-flex flex-wrap gap-2">
            ...
            <form action="{{ route('patients.destroy', $patient->id) }}" method="POST" onsubmit="return confirm(@json(__('Are you sure you want to delete this patient? This action cannot be undone.')));" class="d-inline">
                @csrf
                @method('DELETE')
                <button type="submit" class="btn btn-outline-danger"><i class="fas fa-trash-alt me-1"></i>{{ __('Delete Patient') }}</button>
            </form>
        </div>
    </div>
    ...
    <div class="row justify-content-center">
        <div class="col-12 col-xl-10">
            <form action="{{ route('patients.update', $patient->id) }}" method="POST" enctype="multipart/form-data" class="needs-validation" novalidate data-auto-voice-scope="patient-edit-form">
                @csrf
                @method('PUT')
                ...
                <div class="card border-0 shadow-sm">
                    <div class="card-body p-3 p-lg-4">
                        <div class="alert alert-light border d-flex gap-3 align-items-start mb-4">
                            ...
                            <div>
                                <div class="fw-semibold">{{ __('Modular patient editing') }}</div>
                                <div class="text-muted small mb-0">{{ __('General handles essentials and status, Medical Overview stores shared clinical context, and Modules lets you activate only the specialty profiles this patient needs.') }}</div>
                            </div>
                        </div>

                        <ul class="nav nav-tabs flex-wrap gap-2 border-0 mb-4" role="tablist">
                        ...
                        <div class="tab-content">
                            <div class="tab-pane fade show active" id="general-pane" role="tabpanel">
                                <div class="card bg-light border-0"><div class="card-body"><div class="row g-3">
                                    <div class="col-md-6"><label for="first_name" class="form-label">{{ __('First Name') }} <span class="text-danger">*</span></label><input type="text" class="form-control @error('first_name') is-invalid @enderror" id="first_name" name="first_name" value="{{ old('first_name', $patient->first_name) }}" required>@error('first_name')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
                                    <div class="col-md-6"><label for="last_name" class="form-label">{{ __('Last Name') }} <span class="text-danger">*</span></label><input type="text" class="form-control @error('last_name') is-invalid @enderror" id="last_name" name="last_name" value="{{ old('last_name', $patient->last_name) }}" required>@error('last_name')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
                                    <div class="col-md-6"><label for="date_of_birth" class="form-label">{{ __('Date of Birth') }}</label><input type="date" class="form-control @error('date_of_birth') is-invalid @enderror" id="date_of_birth" name="date_of_birth" value="{{ old('date_of_birth', $patient->date_of_birth_for_form) }}">@error('date_of_birth')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
                                    <div class="col-md-6"><label for="gender" class="form-label">{{ __('Gender') }}</label><select class="form-select @error('gender') is-invalid @enderror" id="gender" name="gender"><option value="">{{ __('Select Gender') }}</option><option value="male" @selected(old('gender', $patient->gender) === 'male')>{{ __('Male') }}</option><option value="female" @selected(old('gender', $patient->gender) === 'female')>{{ __('Female') }}</option><option value="other" @selected(old('gender', $patient->gender) === 'other')>{{ __('Other') }}</option></select>@error('gender')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
                                    <div class="col-md-6"><label for="phone" class="form-label">{{ __('Phone') }}</label><input type="tel" class="form-control @error('phone') is-invalid @enderror" id="phone" name="phone" value="{{ old('phone', $patient->phone) }}">@error('phone')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
                                    <div class="col-md-6"><label for="whatsapp_phone" class="form-label">{{ __('WhatsApp') }}</label><input type="tel" class="form-control @error('whatsapp_phone') is-invalid @enderror" id="whatsapp_phone" name="whatsapp_phone" value="{{ old('whatsapp_phone', $patient->whatsapp_phone) }}">@error('whatsapp_phone')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
                                    <div class="col-md-6"><label for="email" class="form-label">{{ __('Email') }}</label><input type="email" class="form-control @error('email') is-invalid @enderror" id="email" name="email" value="{{ old('email', $patient->email) }}">@error('email')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
                                    <div class="col-md-6"><label for="blood_type" class="form-label">{{ __('Blood Type') }}</label><select class="form-select @error('blood_type') is-invalid @enderror" id="blood_type" name="blood_type"><option value="">{{ __('Select Blood Type') }}</option>@foreach(['NA' => __('NA - Not available'), 'A+' => 'A+', 'A-' => 'A-', 'B+' => 'B+', 'B-' => 'B-', 'AB+' => 'AB+', 'AB-' => 'AB-', 'O+' => 'O+', 'O-' => 'O-'] as $value => $label)<option value="{{ $value }}" @selected(old('blood_type', $patient->blood_type) === $value)>{{ $label }}</option>@endforeach</select>@error('blood_type')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
                                    <div class="col-12"><label for="address" class="form-label">{{ __('Address') }}</label><textarea class="form-control @error('address') is-invalid @enderror" id="address" name="address" rows="3">{{ old('address', $patient->address) }}</textarea>@error('address')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
                                    <div class="col-md-6"><label for="job" class="form-label">{{ __('Occupation') }}</label><input type="text" class="form-control @error('job') is-invalid @enderror" id="job" name="job" value="{{ old('job', $patient->job) }}">@error('job')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
                                    <div class="col-md-6"><label for="education" class="form-label">{{ __('Education Level') }}</label><input type="text" class="form-control @error('education') is-invalid @enderror" id="education" name="education" value="{{ old('education', $patient->education) }}">@error('education')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
                                    <div class="col-md-6"><label for="emergency_contact_name" class="form-label">{{ __('Emergency Contact Name') }}</label><input type="text" class="form-control @error('emergency_contact_name') is-invalid @enderror" id="emergency_contact_name" name="emergency_contact_name" value="{{ old('emergency_contact_name', $patient->emergency_contact_name) }}">@error('emergency_contact_name')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
                                    ...
                                    <div class="col-md-6"><label for="is_active" class="form-label">{{ __('Status') }}</label><select class="form-select @error('is_active') is-invalid @enderror" id="is_active" name="is_active"><option value="1" @selected(old('is_active', $patient->is_active ? '1' : '0') === '1')>{{ __('Active') }}</option><option value="0" @selected(old('is_active', $patient->is_active ? '1' : '0') === '0')>{{ __('Inactive') }}</option></select>@error('is_active')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
                                    <div class="col-12"><label for="notes" class="form-label">{{ __('Notes') }}</label><textarea class="form-control @error('notes') is-invalid @enderror" id="notes" name="notes" rows="3">{{ old('notes', $patient->notes) }}</textarea>@error('notes')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
                                </div></div></div>
                            </div>

                            <div class="tab-pane fade" id="medical-pane" role="tabpanel">
                                <div class="alert alert-info"><i class="fas fa-circle-info me-1"></i>{{ __('HPI stays per visit. Use this section only for shared cross-module medical context and uploaded history files.') }}</div>
                                <div class="accordion" id="medical-overview-accordion">
                                    <div class="accordion-item border rounded-3 mb-3 overflow-hidden"><h2 class="accordion-header"><button class="accordion-button" type="button" data-bs-toggle="collapse" data-bs-target="#collapse-shared-history">{{ __('Shared history') }}</button></h2><div id="collapse-shared-history" class="accordion-collapse collapse show" data-bs-parent="#medical-overview-accordion"><div class="accordion-body row g-3"><div class="col-12"><label for="allergies" class="form-label">{{ __('Allergies') }}</label><textarea class="form-control @error('allergies') is-invalid @enderror" id="allergies" name="allergies" rows="2">{{ old('allergies', data_get($patient->medicalOverview, 'allergies', $patient->allergies)) }}</textarea>@error('allergies')<div class="invalid-feedback">{{ $message }}</div>@enderror</div><div class="col-12"><label for="chronic_illnesses" class="form-label">{{ __('Chronic Diseases') }}</label><textarea class="form-control @error('chronic_illnesses') is-invalid @enderror" id="chronic_illnesses" name="chronic_illnesses" rows="2">{{ old('chronic_illnesses', data_get($patient->medicalOverview, 'chronic_diseases', $patient->chronic_illnesses)) }}</textarea>@error('chronic_illnesses')<div class="invalid-feedback">{{ $message }}</div>@enderror</div><div class="col-12"><label for="current_medications_summary" class="form-label">{{ __('Current Medications') }}</label><textarea class="form-control @error('current_medications_summary') is-invalid @enderror" id="current_medications_summary" name="current_medications_summary" rows="2">{{ old('current_medications_summary', data_get($patient->medicalOverview, 'current_medications_summary')) }}</textarea>@error('current_medications_summary')<div class="invalid-feedback">{{ $message }}</div>@enderror</div></div></div></div>
                                    <div class="accordion-item border rounded-3 mb-3 overflow-hidden"><h2 class="accordion-header"><button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse-procedures">{{ __('Procedures, files & notes') }}</button></h2><div id="collapse-procedures" class="accordion-collapse collapse" data-bs-parent="#medical-overview-accordion"><div class="accordion-body row g-3"><div class="col-12"><label for="surgeries_history" class="form-label">{{ __('Surgeries') }}</label><textarea class="form-control @error('surgeries_history') is-invalid @enderror" id="surgeries_history" name="surgeries_history" rows="2">{{ old('surgeries_history', data_get($patient->medicalOverview, 'surgeries', $patient->surgeries_history)) }}</textarea>@error('surgeries_history')<div class="invalid-feedback">{{ $message }}</div>@enderror</div><div class="col-12"><label for="medical_history" class="form-label">{{ __('Medical History') }}</label><textarea class="form-control @error('medical_history') is-invalid @enderror" id="medical_history" name="medical_history" rows="3">{{ old('medical_history', data_get($patient->medicalOverview, 'medical_history', $patient->medical_history)) }}</textarea>@error('medical_history')<div class="invalid-feedback">{{ $message }}</div>@enderror</div><div class="col-12"><label for="medical_files" class="form-label">{{ __('Medical History Files') }}</label><input type="file" class="form-control @error('medical_files.*') is-invalid @enderror" id="medical_files" name="medical_files[]" multiple accept=".pdf,.jpg,.jpeg,.png,.doc,.docx"><small class="text-muted">{{ __('Upload reports, scans, or intake-related documents.') }}</small>@error('medical_files.*')<div class="invalid-feedback">{{ $message }}</div>@enderror</div></div></div></div>
                                    <div class="accordion-item border rounded-3 overflow-hidden"><h2 class="accordion-header"><button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse-flags">{{ __('Flags') }}</button></h2><div id="collapse-flags" class="accordion-collapse collapse" data-bs-parent="#medical-overview-accordion"><div class="accordion-body"><div class="row g-3">@foreach(($medicalFlags ?? []) as $flagKey => $flagLabel)<div class="col-sm-6 col-lg-4"><div class="form-check border rounded p-3 h-100"><input class="form-check-input" type="checkbox" id="medical_flag_{{ $flagKey }}" name="medical_flags[{{ $flagKey }}]" value="1" @checked(old('medical_flags.' . $flagKey, data_get($medicalFlagsState, $flagKey, false)))><label class="form-check-label fw-semibold" for="medical_flag_{{ $flagKey }}">{{ __($flagLabel) }}</label></div></div>@endforeach</div></div></div></div>
                                </div>
                            </div>

                            <div class="tab-pane fade" id="modules-pane" role="tabpanel">
                                <div class="card bg-light border-0 mb-3"><div class="card-body d-flex flex-column flex-lg-row justify-content-between gap-3 align-items-lg-center"><div><h6 class="mb-1">{{ __('Optional modules') }}</h6><p class="text-muted mb-0 small">{{ __('Turn specialty modules on or off here. Removing a module only deactivates it from the patient profile; it does not delete specialty records.') }}</p></div><div class="d-flex flex-column flex-sm-row gap-2"><button type="button" class="btn btn-outline-primary" data-add-module-toggle><i class="fas fa-plus me-1"></i>{{ __('Add Module') }}</button><div class="d-none" id="module-selector-wrap"><select class="form-select" id="module-selector"><option value="">{{ __('Choose module') }}</option>@foreach($moduleDefinitions as $module)<option value="{{ $module['key'] }}">{{ $module['label'] }}</option>@endforeach</select></div></div></div></div>
                                    ...
                                    @foreach($moduleDefinitions as $module)
                                        ...
                                        <div class="col-12 col-lg-6 module-card {{ $isSelected ? '' : 'd-none' }}" data-module-card="{{ $module['key'] }}"><div class="card border shadow-sm h-100"><div class="card-header bg-white d-flex justify-content-between align-items-start gap-3"><div><div class="fw-semibold"><i class="{{ $module['icon'] }} text-primary me-2"></i>{{ $module['label'] }}</div><div class="small text-muted">{{ $module['description'] }}</div></div><button type="button" class="btn btn-sm btn-outline-danger" data-remove-module="{{ $module['key'] }}">{{ __('Remove') }}</button></div><div class="card-body"><input type="hidden" name="selected_modules[]" value="{{ $module['key'] }}" class="selected-module-input" {{ $isSelected ? '' : 'disabled' }}>
                                            @if($module['key'] === 'dental')
                                                <div class="row g-3"><div class="col-md-6"><label for="dental_oral_hygiene" class="form-label">{{ __('Oral Hygiene') }}</label><select id="dental_oral_hygiene" name="dental_oral_hygiene" class="form-select module-field @error('dental_oral_hygiene') is-invalid @enderror" {{ $isSelected ? '' : 'disabled' }}><option value="">{{ __('Select status') }}</option>@foreach(($dentalOralHygieneOptions ?? []) as $value => $label)<option value="{{ $value }}" @selected(old('dental_oral_hygiene', data_get($patient->dentalProfile, 'oral_hygiene')) === $value)>{{ __($label) }}</option>@endforeach</select>@error('dental_oral_hygiene')<div class="invalid-feedback">{{ $message }}</div>@enderror</div><div class="col-md-6"><label for="dental_smoking_status" class="form-label">{{ __('Smoking Status') }}</label><select id="dental_smoking_status" name="dental_smoking_status" class="form-select module-field @error('dental_smoking_status') is-invalid @enderror" {{ $isSelected ? '' : 'disabled' }}><option value="">{{ __('Select status') }}</option>@foreach(($dentalSmokingStatusOptions ?? []) as $value => $label)<option value="{{ $value }}" @selected(old('dental_smoking_status', data_get($patient->dentalProfile, 'smoking_status')) === $value)>{{ __($label) }}</option>@endforeach</select>@error('dental_smoking_status')<div class="invalid-feedback">{{ $message }}</div>@enderror</div></div>
                                            @elseif($module['key'] === 'pediatric')
                                                <div class="row g-3"><div class="col-md-6"><label for="pediatric_birth_weight" class="form-label">{{ __('Birth Weight') }}</label><input type="number" id="pediatric_birth_weight" name="pediatric_birth_weight" min="200" max="7000" step="1" class="form-control module-field @error('pediatric_birth_weight') is-invalid @enderror" value="{{ old('pediatric_birth_weight', data_get($patient->pediatricProfile, 'birth_weight', $patient->birth_weight)) }}" {{ $isSelected ? '' : 'disabled' }}>@error('pediatric_birth_weight')<div class="invalid-feedback">{{ $message }}</div>@enderror</div><div class="col-md-6"><label for="pediatric_gestational_age_weeks" class="form-label">{{ __('Gestational Age') }}</label><input type="number" id="pediatric_gestational_age_weeks" name="pediatric_gestational_age_weeks" min="20" max="45" step="1" class="form-control module-field @error('pediatric_gestational_age_weeks') is-invalid @enderror" value="{{ old('pediatric_gestational_age_weeks', data_get($patient->pediatricProfile, 'gestational_age', $patient->gestational_age_weeks)) }}" {{ $isSelected ? '' : 'disabled' }}>@error('pediatric_gestational_age_weeks')<div class="invalid-feedback">{{ $message }}</div>@enderror</div><div class="col-12 d-none" id="pediatric-status-indicator"><div id="pediatric-status-message" class="alert py-2 mb-0"></div></div></div>
                                            @elseif($module['key'] === 'nutrition')
                                                <div class="row g-3"><div class="col-md-6"><label for="nutrition_height" class="form-label">{{ __('Height (cm)') }}</label><input type="number" id="nutrition_height" name="nutrition_height" min="50" max="300" step="0.1" class="form-control module-field @error('nutrition_height') is-invalid @enderror" value="{{ old('nutrition_height', data_get($patient->nutritionProfile, 'height', $patient->height)) }}" {{ $isSelected ? '' : 'disabled' }}>@error('nutrition_height')<div class="invalid-feedback">{{ $message }}</div>@enderror</div><div class="col-md-6"><label for="nutrition_weight" class="form-label">{{ __('Weight (kg)') }}</label><input type="number" id="nutrition_weight" name="nutrition_weight" min="1" max="500" step="0.1" class="form-control module-field @error('nutrition_weight') is-invalid @enderror" value="{{ old('nutrition_weight', data_get($patient->nutritionProfile, 'weight', $patient->weight)) }}" {{ $isSelected ? '' : 'disabled' }}>@error('nutrition_weight')<div class="invalid-feedback">{{ $message }}</div>@enderror</div></div>
                                            @elseif($module['key'] === 'ent')
                                                <div><label for="ent_notes" class="form-label">{{ __('Notes') }}</label><textarea id="ent_notes" name="ent_notes" rows="4" class="form-control module-field @error('ent_notes') is-invalid @enderror" {{ $isSelected ? '' : 'disabled' }}>{{ old('ent_notes', data_get($patient->entProfile, 'notes')) }}</textarea>@error('ent_notes')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
                                            @endif
                                        </div></div></div>
                                    @endforeach
                                </div>
                        ...
                        <div class="d-flex flex-column flex-sm-row justify-content-end gap-2 mt-4 pt-3 border-top"><a href="{{ route('patients.show', $patient->id) }}" class="btn btn-outline-secondary">{{ __('Cancel') }}</a><button type="submit" class="btn btn-primary"><i class="fas fa-save me-1"></i>{{ __('Update Patient') }}</button></div>
                    </div>
                </div>
            </form>
        </div>
