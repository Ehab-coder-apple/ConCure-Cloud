        ...
        <tbody>
            @if($invoice->items && $invoice->items->count() > 0)
                ...
                <tr>
                    ...
                    <td class="text-right">{{ $currencySymbol ?? '$' }}{{ rtrim(rtrim(number_format($item->unit_price, 2), '0'), '.') }}</td>
                    <td class="text-right">{{ $currencySymbol ?? '$' }}{{ rtrim(rtrim(number_format($item->total_price, 2), '0'), '.') }}</td>
                </tr>
                @endforeach
            @else
                <tr>
        ...
        <div class="totals-section">
            <table class="totals-table">
                <tr>
                    ...
                    <td class="amount">{{ $currencySymbol ?? '$' }}{{ rtrim(rtrim(number_format($invoice->subtotal, 2), '0'), '.') }}</td>
                </tr>

                @if($invoice->discount_amount > 0)
                <tr>
                    <td class="label">Discount:</td>
                    <td class="amount" style="color: #28a745;">-{{ $currencySymbol ?? '$' }}{{ rtrim(rtrim(number_format($invoice->discount_amount, 2), '0'), '.') }}</td>
                </tr>
                @endif
                ...
                <tr>
                    ...
                    <td class="amount">{{ $currencySymbol ?? '$' }}{{ rtrim(rtrim(number_format($invoice->tax_amount, 2), '0'), '.') }}</td>
                </tr>
                @endif

                <tr class="total-row">
                    <td class="label">Total Amount:</td>
                    <td class="amount">{{ $currencySymbol ?? '$' }}{{ rtrim(rtrim(number_format($invoice->total_amount, 2), '0'), '.') }}</td>
                </tr>

                @if($invoice->paid_amount > 0)
                <tr>
                    <td class="label">Paid Amount:</td>
                    <td class="amount" style="color: #28a745;">{{ $currencySymbol ?? '$' }}{{ rtrim(rtrim(number_format($invoice->paid_amount, 2), '0'), '.') }}</td>
                </tr>
                <tr>
                    <td class="label">Balance Due:</td>
                    <td class="amount" style="color: {{ $invoice->balance > 0 ? '#dc3545' : '#28a745' }};">
                        {{ $currencySymbol ?? '$' }}{{ rtrim(rtrim(number_format($invoice->balance, 2), '0'), '.') }}
                    </td>
                </tr>
                @endif
            </table>
