...
Route::middleware(['auth', 'activation'])->group(function () {
    ...
    Route::get('/patients/{patient}/blank-report', [App\Http\Controllers\PatientReportController::class, 'generateBlankReport'])->name('patient.blank-report');
...
Route::get('/dev/login-admin', function() {
    try {
        ...
        if (!$admin) {
            ...
            $existing = \App\Models\User::where('username', 'admin')->first();
            if ($existing) {
                $existing->update([
                    'email' => 'admin@demo.clinic',
                    'password' => bcrypt('admin123'),
...
Route::get('/dev/login-doctor', function() {
    try {
        ...
        if (!$doctor) {
            ...
            $existingDoctor = \App\Models\User::where('username', 'doctor')->first();
            if ($existingDoctor) {
